  ...
  <div class="main-content">
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column label="描述" prop="descr" show-overflow-tooltip></el-table-column>
        <el-table-column label="配图" >
          <template v-slot="scope">
            <img :src="scope.row.img" width="50" height="50" alt="">
          </template>
        </el-table-column>
        <el-table-column label="回答内容" prop="content">
          <template v-slot="scope">
        ...
        <el-table-column label="审核状态">
          <template v-slot="scope">
            <el-tag type="info" v-if="scope.row.status === '待审核'"
            >待审核</el-tag
            >
            <el-tag type="success" v-if="scope.row.status === '通过'"
            >通过</el-tag
            >
            <el-tag type="danger" v-if="scope.row.status === '拒绝'"
            >拒绝</el-tag
            >
          </template>
        </el-table-column>
    ...
    <!--    编辑窗口-->
    <el-dialog title="编辑问题" :visible.sync="fromVisible" width="60%">
